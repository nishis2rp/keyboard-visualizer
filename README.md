...
本プロジェクトには、アプリケーションの魅力を伝える洗練されたランディングページが追加されています。

### デザイン特徴

- **動的な幾何学背景**: Canvas APIを使用した80個のパーティクル（点）が画面上を移動し、近接する点同士が線で接続されるアニメーション
- **ダークテーマ**: 深い紫系のグラデーション背景（`#0f0c29 → #302b63 → #24243e`）に、パルスグローエフェクト
- **グラスモーフィズムUI**: 半透明カードに`backdrop-filter: blur()`を適用した最新のUIデザイン
- **グラデーションタイトル**: "KEYBOARD VISUALIZER"に白→ライトブルー→紫のグラデーションと脈動するグロー効果

### コンテンツセクション

1. **ヒーローセクション**: メインタイトルとCTAボタン
2. **統計情報**: 1,300+ショートカット、9対応アプリ、4難易度レベル
3. **機能紹介**: Visual Feedback、Active Learning、Master Every Appの3つの主要機能
4. **対応アプリ一覧**: 9つのアプリケーションアイコンをグリッド表示
5. **メリット**: 生産性向上、フロー状態維持、プロスキル習得、身体負担軽減の4つのベネフィット
6. **使い方**: 3ステップのフローチャート（アプリ選択 → キー入力 → フィードバック）
7. **最終CTA**: 行動を促す大きなボタンでアプリへ誘導

### 技術実装

- **React Hooks**: `useEffect`と`useRef`でCanvas APIを制御
- **パーティクルシステム**: 物理エンジン（境界での反射、速度ベクトル）を実装
- **レスポンシブデザイン**: モバイル対応のメディアクエリとフレキシブルレイアウト
- **パフォーマンス最適化**: `requestAnimationFrame`による滑らかなアニメーション

ランディングページは `/` ルートでアクセス可能で、`/app` ルートでメインアプリケーションが起動します。
...
- **難易度フィルター**: ビジュアライザーモードで難易度別にショートカットを表示可能（複数選択可）
  - 🌟 Basic、⚡ Standard、🔥 Hard、💀 Madmax から自由に組み合わせ
...
- **スマートな出題フィルタリング**: システム保護ショートカット（Alt+F4、Cmd+Qなど）は自動的に除外され、安全に学習できる
...
- **複数の認証方法**: Google OAuth、GitHub OAuth、メール/パスワード認証に対応
- **クイズ進捗の保存**: ログインユーザーのクイズセッション、スコア、回答履歴を自動保存
- **統計データの追跡**: 正解率、平均速度、総回答数などの詳細な学習データを記録
- **マイページ機能**:
  - 📊 **クイズ統計**: アプリケーション別の正解率、セッション数、最終プレイ日時を表示
  - 📝 **セッション履歴**: 最近のクイズセッション（スコア、難易度、完了日時）を一覧表示
  - 🔍 **詳細モーダル**: 各セッションの回答履歴（正解/不正解、ショートカット、回答時刻）を確認
  - 👤 **プロフィール管理**: 表示名とアバター画像のカスタマイズ
  - 🔑 **アカウント設定**: メールアドレス変更、パスワードリセット、アカウント削除
...
3. **プロフィール管理**
   - ログイン後、ヘッダーの表示名をクリック
   - 「マイページ」で統計とプロフィール設定を表示
   - 「ログアウト」でセッションを終了

4. **マイページ**
   - ヘッダーのユーザーメニューから「マイページ」を選択
   - クイズ統計、セッション履歴、プロフィール設定、アカウント設定を確認・編集
...
13. **UI/UXの改善とレイアウト最適化**（2026年2月）
   - ヘッダー領域の説明テキストを削除し、縦方向にコンパクト化
   - 難易度フィルターを右側に配置、押されたキー表示を左側に配置する横並びレイアウトに変更
   - 押されたキー表示エリアに視認性の高い紫枠を追加し、フィルターと高さを統一
   - ショートカット候補一覧を下部に配置し、情報の階層を明確化
   - `KeyDisplay` コンポーネントを `PressedKeysIndicator` と `ShortcutsList` に分割し、関心の分離を実現
   - 全体的なコンパクト化により、画面領域の有効活用と視認性の向上

14. **SEO最適化とアナリティクス統合**（2026年2月）
   - robots.txtとsitemap.xmlの追加による検索エンジン最適化
   - Google Tag Manager / GA4対応のアナリティクスユーティリティを実装
   - クイズ開始・完了、アプリ選択、ショートカット習得などのユーザー行動を追跡
   - プロダクション環境のみでトラッキング実行（開発環境では非アクティブ）
   - 検索エンジンでの発見性向上とユーザー行動分析の基盤構築

15. **UI/レイアウトの最適化**（2026年2月）
   - ヘッダータイトルを"KEYBOARD Visualizer"として1行表示に変更
   - アプリケーション選択とキーボード配列選択を12グリッドシステム（9:3比率）で1行に統一
   - 各グリッドアイテムの幅を統一し、視覚的な一貫性を向上
   - 990px幅でのレイアウト最適化により、画面領域の効率的な利用を実現
   - セットアップ画面のアイコンをカラーSVGに統一（#6366f1ベースのインディゴブルー）
   - 絵文字からSVGアイコンへの移行により、全体的なデザインの一貫性を確保

16. **SVGアイコンパスの動的解決**（2026年2月）
   - `AppIcon.tsx`でViteの`import.meta.env.BASE_URL`を使用してベースパスを動的に取得
   - GitHub Pages（`/keyboard-visualizer/`）とローカル環境（`/`）の両方で正しくSVGアイコンが表示されるように修正
   - ハードコードされたパスから環境変数ベースの柔軟な実装に改善
   - セットアップ画面、難易度フィルター等の全てのアイコン表示が環境に依存せず動作

17. **ランディングページの追加**（2026年2月）
   - Canvas APIによる動的な幾何学背景（80個のパーティクルと線のアニメーション）
   - ダークテーマの洗練されたグラスモーフィズムデザイン
   - 統計情報、機能紹介、対応アプリ一覧、メリット、使い方の5つのセクション
   - React Routerによる`/`（LP）と`/app`（メインアプリ）のルーティング
