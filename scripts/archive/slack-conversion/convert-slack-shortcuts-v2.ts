...
const normalizeKeyCombo = (keyCombo: string): string => {
  const modifierOrder: Record<string, number> = { 'Ctrl': 1, 'Shift': 2, 'Alt': 3, 'Win': 4, 'Control': 1, 'Meta': 4 };
...
const shortcuts: Record<string, CategoryAction[]> = {}; // キー -> [{category, action}] の形式
