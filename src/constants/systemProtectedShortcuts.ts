/**
 * 常に保護されるショートカットのリスト。
 * これらのショートカットは、どのモードでもアプリケーションによって上書きされるべきではありません。
 * 主にOSレベルの重要なショートカットや、ブラウザの終了・タブ操作など、ユーザー体験に重大な影響を与えるショートカットが含まれます。
 */
export const ALWAYS_PROTECTED_SHORTCUTS = new Set([
  // OSレベルの重要なショートカット
  'Alt + F4',       // アプリケーション終了 (Windows)
  'Meta + L',       // 画面ロック (Windows: Win+L) / ロック (macOS: Cmd+L) - ブラウザでは通常Cmd+Lでアドレスバーにフォーカスだが、OSによってはロック機能
  'Meta + Q',       // アプリケーション終了 (macOS)
  'Ctrl + Alt + Delete', // タスクマネージャー (Windows)
  'Meta + Pause',    // システム情報ダイアログ (Windows)

  // ブラウザのタブ/ウィンドウ操作など、誤操作による影響が大きいもの
  'Ctrl + Shift + T', // 閉じたタブを再度開く
  'Ctrl + Shift + W', // ウィンドウを閉じる
]);

/**
 * フルスクリーンモードでない場合に、ブラウザのデフォルト動作が妨げられる可能性のあるショートカットのリスト。
 * これらのショートカットは、フルスクリーンモードでは安全と見なされますが、
 * 通常モードではブラウザの重要な機能を実行するため、アプリケーションがクイズなどで上書きしない方が良いとされます。
 */
